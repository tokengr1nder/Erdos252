...
\end{itemize}
Proof module: \textbf{834 substantive lines; 1,120 total lines}.
...
publication verification receipts review-final-0912 in the local build directory.
...
\begin{frame}{Appendix: one numerator bound controls both errors}
...
\[
 0\le\sigma_k(x)F_{j,k+1}(x)
 \le\frac{64(k+1)^{k+1}}{x\sqrt{n+1}}
 \le\frac{64(k+1)^{k+1}}{(n+1)^{3/2}}.
\]
For the omitted terms, factorial denominators yield a geometric majorant;
their total is at most $64(k+1)^{k+1}/(n\sqrt{n+1})$ for $n>0$.
There are $k+1$ retained errors. Hence, for sufficiently large $n$,
\[
 E_n\le
 \frac{64(k+1)^{k+1}}{n\sqrt{n+1}}+
 \frac{64(k+1)^{k+2}}{(n+1)^{3/2}}.
\]
...
\lean{PROOF.pdf} contains all 94 named proof results and their proofs,
40 definitions, and the six statement-audit results.
