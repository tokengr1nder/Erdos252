...
theorem summable_sigma_factorial (k : ℕ) : Summable (fun n : ℕ ↦ (σ k n : ℝ) / (n ! : ℝ)) := by
  refine .of_nonneg_of_le (fun n ↦ by positivity) (fun n ↦ ?_) (Real.summable_pow_div_factorial ((2 : ℝ) ^ (k + 1)))
  gcongr
  exact_mod_cast (ArithmeticFunction.sigma_le_pow_succ k n).trans
    (by simpa only [← pow_mul, Nat.mul_comm] using Nat.pow_le_pow_left (Nat.lt_two_pow_self (n := n)).le (k + 1) : n ^ (k + 1) ≤ (2 ^ (k + 1)) ^ n)

theorem eventually_zero_of_int {f : ℕ → ℝ} (hf : Tendsto f atTop (𝓝 0)) (hint : ∀ᶠ n in atTop, ∃ z : ℤ, f n = z) : ∀ᶠ n in atTop, f n = 0 := by
  filter_upwards [hint, ((tendsto_zero_iff_abs_tendsto_zero _).mp hf).eventually (gt_mem_nhds one_pos)] with n ⟨z, hz⟩ hs
...
private theorem card_divisors_le_sqrt (n : ℕ) (hn : 0 < n) : n.divisors.card ≤ 2 * Nat.sqrt n := by
  ...
  have hcover : n.divisors ⊆ S ∪ S.image (fun d => n / d) := by
    ...
    · apply Finset.mem_union_right
      exact Finset.mem_image.mpr ⟨n / d, Finset.mem_Icc.mpr ⟨Nat.div_pos (Nat.le_of_dvd hn hdvd) hdpos, hsmall⟩, Nat.div_div_self hdvd hn.ne'⟩
  calc
...
theorem sigma_le_pow_sqrt (k n : ℕ) (hn : 0 < n) : (σ k n : ℝ) ≤ 64 * (n : ℝ) ^ k * √(n : ℝ) := by
  have hnat : σ k n ≤ n.divisors.card * n ^ k := by
    simpa only [ArithmeticFunction.sigma_apply, Finset.sum_const, nsmul_eq_mul, Nat.cast_id] using Finset.sum_le_sum (s := n.divisors)
  ...
  simpa only [mul_assoc, mul_comm, mul_left_comm] using mul_le_mul_of_nonneg_right hcard (pow_nonneg (Nat.cast_nonneg n) k)
...
theorem tendsto_sqrt_div_nat : Tendsto (fun n : ℕ => √(n : ℝ) / (n : ℝ)) atTop (𝓝 0) := by
  simpa only [Real.sqrt_div_self, Function.comp_def] using tendsto_inv_atTop_zero.comp (Real.tendsto_sqrt_atTop.comp tendsto_natCast_atTop_atTop)
...
def stirlingPoly (j L : ℕ) (x : ℝ) : ℝ := ∑ i ∈ Finset.range L, (Nat.stirlingSecond i j : ℝ) / x ^ (i + 1)

/-- The exact remainder after the finite Stirling expansion. -/
def stirlingErr (j L : ℕ) (x : ℝ) : ℝ := descRecip (j + 1) x - stirlingPoly j L x

theorem descRecip_succ (h : ℕ) (x : ℝ) : descRecip (h + 1) x = descRecip h x / (x - (h : ℝ)) := by simp only [descRecip, Finset.prod_range_succ, div_div]

theorem descRecip_one (x : ℝ) : descRecip 1 x = 1 / x := by simp [descRecip]

theorem stirlingPoly_one (L : ℕ) (x : ℝ) : stirlingPoly 0 (L + 1) x = 1 / x := by simp [stirlingPoly, Finset.sum_range_succ', Nat.stirlingSecond]

/-- The finite coefficient recurrence holds at every truncation order. -/
theorem stirlingPoly_recurrence (j L : ℕ) (x : ℝ) : stirlingPoly (j + 1) (L + 1) x = (stirlingPoly j L x + ((j : ℝ) + 1) * stirlingPoly (j + 1) L x) / x := by
  unfold stirlingPoly
...
theorem stirlingErr_recurrence (j L : ℕ) (x : ℝ) (hx : x ≠ 0) (hxj : x - ((j : ℝ) + 1) ≠ 0) : stirlingErr (j + 1) (L + 1) x =
...
theorem descRecip_bounds (h H : ℕ) (x : ℝ) (hh : 1 ≤ h) (hH : h ≤ H) (hx : (H : ℝ) ≤ x) : 0 < descRecip h x ∧ descRecip h x ≤ 1 / x := by
...
theorem stirlingErr_bounds (H L : ℕ) (x : ℝ) (hx : (H : ℝ) ≤ x) (j : ℕ) (hH : j + 1 ≤ H) :
  ...
  | succ L ih =>
    ...
    calc
      _ ≤ ((H : ℝ) ^ L / x ^ (L + 1) + ((j : ℝ) + 1) * ((H : ℝ) ^ L / x ^ (L + 1))) / x := (div_le_div_iff_of_pos_right hxpos).mpr
...
theorem descRecip_centered (n h : ℕ) : descRecip h ((n + h : ℕ) : ℝ) = 1 / ((n + 1).ascFactorial h : ℝ) := by
  unfold descRecip
...
def seriesPrefix (k n : ℕ) : ℝ := ∑ m ∈ Finset.range n, (σ k m : ℝ) / (m.factorial : ℝ)
...
def tailMain (k n : ℕ) : ℝ := ∑ j ∈ Finset.range (k + 1), (σ k (n + (j + 1)) : ℝ) * stirlingPoly j (k + 1) ((n + (j + 1) : ℕ) : ℝ)
...
def finiteErr (k n : ℕ) : ℝ := ∑ j ∈ Finset.range (k + 1), (σ k (n + (j + 1)) : ℝ) * stirlingErr j (k + 1) ((n + (j + 1) : ℕ) : ℝ)

/-- The factorial-scaled actual prefix is integral for every exponent. -/
theorem seriesPrefix_integral (k n : ℕ) : ∃ z : ℤ, ((n - 1).factorial : ℝ) * seriesPrefix k n = z := by
...
theorem eventually_scaledTail_integral (k : ℕ) (hx : ¬ Irrational (alpha k)) : ∃ N : ℕ, ∀ n : ℕ, N ≤ n → ∃ z : ℤ, scaledTail k n = z := by
...
private theorem factorial_ratio (n j : ℕ) (hn : 0 < n) : ((n - 1).factorial : ℝ) / ((n + j).factorial : ℝ) = 1 / (n.ascFactorial (j + 1) : ℝ) := by
  obtain ⟨m, rfl⟩ := Nat.exists_eq_succ_of_ne_zero hn.ne'
  have ha : ((m + 1).ascFactorial (j + 1) : ℝ) ≠ 0 := by positivity
  field_simp
  norm_cast
  simpa only [Nat.succ_eq_add_one, Nat.add_sub_cancel, Nat.add_assoc, Nat.add_comm 1] using Nat.factorial_mul_ascFactorial m (j + 1)

private theorem scaled_summand_eq_blockTerm (k n j : ℕ) (hn : 0 < n) : ((n - 1).factorial : ℝ) * ((σ k (j + n) : ℝ) / ((j + n).factorial : ℝ)) =
      blockTerm k n j := by
  rw [Nat.add_comm j n]
  unfold blockTerm
...
theorem summable_blockTerm (k n : ℕ) (hn : 0 < n) : Summable (blockTerm k n) := (((summable_nat_add_iff n).2 (summable_sigma_factorial k)).mul_left
    ((n - 1).factorial : ℝ)).congr (fun j => scaled_summand_eq_blockTerm k n j hn)

theorem blockTerm_nonneg (k n j : ℕ) : 0 ≤ blockTerm k n j := div_nonneg (Nat.cast_nonneg _) (Nat.cast_nonneg _)

theorem omittedTail_nonneg (k n H : ℕ) : 0 ≤ omittedTail k n H := tsum_nonneg (fun r => blockTerm_nonneg k (n + 1) (r + H))

/-- Splitting the actual tail at any finite order. -/
theorem scaledTail_split (k n H : ℕ) : scaledTail k (n + 1) = (∑ j ∈ Finset.range H, blockTerm k (n + 1) j) + omittedTail k n H := by
...
theorem blockTerm_expansion (k n j L : ℕ) : blockTerm k (n + 1) j = (σ k (n + (j + 1)) : ℝ) * stirlingPoly j L ((n + (j + 1) : ℕ) : ℝ) +
...
theorem scaledTail_expansion (k n : ℕ) : scaledTail k (n + 1) = tailMain k n + tailErr k n := (add_sub_cancel _ _).symm

/-- The finite main term with both indices in the zero-based convention. -/
theorem tailMain_eq_range (k n : ℕ) : tailMain k n = ∑ j ∈ Finset.range (k + 1), ∑ i ∈ Finset.range (k + 1), (Nat.stirlingSecond i j : ℝ) *
          (σ k (n + (j + 1)) : ℝ) / ((n + (j + 1) : ℕ) : ℝ) ^ (i + 1) := by
  unfold tailMain stirlingPoly
  simp_rw [Finset.mul_sum]
  simp only [div_eq_mul_inv, mul_comm, mul_assoc, mul_left_comm]

theorem poly_ascending_bound (d n r : ℕ) (hd : 0 < d) : (n + 1 + (r + d)) ^ d * (n + 1) ^ (r + 1) ≤ d ^ d * (n + 1).ascFactorial (r + d + 1) := by
  ...
  calc
    _ ≤ (d ^ d * (n + 1 + (r + 1)).ascFactorial d) * (n + 1).ascFactorial (r + 1) := Nat.mul_le_mul hnum (Nat.pow_succ_le_ascFactorial _ _)
    _ = _ := by rw [mul_assoc, Nat.mul_comm ((n + 1 + (r + 1)).ascFactorial d), Nat.ascFactorial_mul_ascFactorial, Nat.add_right_comm r 1 d]

theorem poly_block_le (d n r : ℕ) (hd : 0 < d) : ((n + 1 + (r + d) : ℕ) : ℝ) ^ d / ((n + 1).ascFactorial (r + d + 1) : ℝ) ≤
      (d : ℝ) ^ d / ((n : ℝ) + 1) ^ (r + 1) := by
  have hden : (0 : ℝ) < ((n + 1).ascFactorial (r + d + 1) : ℝ) := by exact_mod_cast Nat.ascFactorial_pos n (r + d + 1)
  apply (div_le_div_iff₀ hden (by positivity)).mpr
  exact_mod_cast poly_ascending_bound d n r hd

theorem sigma_le_pow_succ_div_sqrt (k n m : ℕ) (hnm : n + 1 ≤ m) : (σ k m : ℝ) ≤ (64 / √((n : ℝ) + 1)) * (m : ℝ) ^ (k + 1) := by
  calc
    _ ≤ 64 * (m : ℝ) ^ k * √(m : ℝ) := sigma_le_pow_sqrt k m (by omega)
    _ = 64 * (m : ℝ) ^ (k + 1) / √(m : ℝ) := by simp only [pow_succ, mul_assoc, mul_div_assoc, Real.div_sqrt]
...
theorem omittedTerm_le (k n r : ℕ) : blockTerm k (n + 1) (r + (k + 1)) ≤ (64 / √((n : ℝ) + 1)) * ((k + 1 : ℕ) : ℝ) ^ (k + 1) / ((n : ℝ) + 1) ^ (r + 1) := by
  unfold blockTerm
  refine (div_le_div_of_nonneg_right (sigma_le_pow_succ_div_sqrt k n _ (by omega)) (Nat.cast_nonneg _)).trans ?_
  simpa only [mul_div_assoc] using mul_le_mul_of_nonneg_left (poly_block_le (k + 1) n r (by omega)) (show 0 ≤ (64 : ℝ) / √((n : ℝ) + 1) by positivity)

theorem hasSum_geometric_recip (C : ℝ) (n : ℕ) (hn : 0 < n) : HasSum (fun r : ℕ => C / ((n : ℝ) + 1) ^ (r + 1)) (C / (n : ℝ)) := by
  have hnR : (0 : ℝ) < n := by exact_mod_cast hn
  have hq : (1 : ℝ) / ((n : ℝ) + 1) < 1 := (div_lt_one (by positivity)).mpr (by linarith)
  convert! HasSum.mul_left (C / ((n : ℝ) + 1)) (hasSum_geometric_of_lt_one (by positivity) hq) using 1
  · funext r
    simp only [pow_succ, div_eq_mul_inv, mul_inv_rev]
    ring
  · field_simp [hnR.ne']
...
theorem omittedTail_le (k n : ℕ) (hn : 0 < n) : omittedTail k n (k + 1) ≤ ((64 / √((n : ℝ) + 1)) * ((k + 1 : ℕ) : ℝ) ^ (k + 1)) / (n : ℝ) := by
  have hs : Summable (fun r : ℕ => blockTerm k (n + 1) (r + (k + 1))) := (summable_nat_add_iff (k + 1)).2 (summable_blockTerm k (n + 1) (by omega))
  simpa only [omittedTail, (hasSum_geometric_recip _ n hn).tsum_eq] using hs.tsum_le_tsum (omittedTerm_le k n) (hasSum_geometric_recip _ n hn).summable

/-- The actual omitted tail is negligible after scaling by the base index. -/
theorem omittedTail_scaled_le (k n : ℕ) (hn : 0 < n) : ((n : ℝ) + 1) * omittedTail k n (k + 1) ≤ 128 * ((k + 1 : ℕ) : ℝ) ^ (k + 1) / √((n : ℝ) + 1) := by
  have hnR : (0 : ℝ) < n := by exact_mod_cast hn
  have hn1 : (1 : ℝ) ≤ n := by exact_mod_cast hn
  have hratio : ((n : ℝ) + 1) / (n : ℝ) ≤ 2 := (div_le_iff₀ hnR).mpr (by linarith)
  calc
    _ ≤ ((n : ℝ) + 1) * (((64 / √((n : ℝ) + 1)) * ((k + 1 : ℕ) : ℝ) ^ (k + 1)) / (n : ℝ)) := mul_le_mul_of_nonneg_left (omittedTail_le k n hn) (by positivity)
    _ = (((n : ℝ) + 1) / (n : ℝ)) * ((64 / √((n : ℝ) + 1)) * ((k + 1 : ℕ) : ℝ) ^ (k + 1)) := by ring
    _ ≤ 2 * ((64 / √((n : ℝ) + 1)) * ((k + 1 : ℕ) : ℝ) ^ (k + 1)) := mul_le_mul_of_nonneg_right hratio (by positivity)
    _ = _ := by ring

private theorem sqrt_shift_bound (n h : ℕ) (hh : h ≤ n + 1) : √((n + h : ℕ) : ℝ) ≤ 2 * √((n : ℝ) + 1) := by
...
theorem finiteErr_term_bound (k n j : ℕ) (hn : k + 1 ≤ n) (hj : j < k + 1) : 0 ≤ (σ k (n + (j + 1)) : ℝ) * stirlingErr j (k + 1) ((n + (j + 1) : ℕ) : ℝ) ∧
      (σ k (n + (j + 1)) : ℝ) * stirlingErr j (k + 1) ((n + (j + 1) : ℕ) : ℝ) ≤ 128 * ((k : ℝ) + 1) ^ (k + 1) * √((n : ℝ) + 1) / ((n : ℝ) + 1) ^ 2 := by
  have hx : (0 : ℝ) < ((n + (j + 1) : ℕ) : ℝ) := by exact_mod_cast (show 0 < n + (j + 1) by omega)
  have hXx : (n : ℝ) + 1 ≤ ((n + (j + 1) : ℕ) : ℝ) := by exact_mod_cast (show n + 1 ≤ n + (j + 1) by omega)
  have he := stirlingErr_bounds (k + 1) (k + 1) ((n + (j + 1) : ℕ) : ℝ) (by exact_mod_cast (show k + 1 ≤ n + (j + 1) by omega)) j (by omega)
  ...
  calc
    _ ≤ (64 * ((n + (j + 1) : ℕ) : ℝ) ^ k * √((n + (j + 1) : ℕ) : ℝ)) * (((k + 1 : ℕ) : ℝ) ^ (k + 1) / ((n + (j + 1) : ℕ) : ℝ) ^ (k + 1 + 1)) :=
...
theorem finiteErr_bounds (k n : ℕ) (hn : k + 1 ≤ n) : 0 ≤ finiteErr k n ∧ ((n : ℝ) + 1) * finiteErr k n ≤
...
theorem tendsto_tailErr_mul (k : ℕ) : Tendsto (fun n : ℕ => ((n : ℝ) + 1) * tailErr k n) atTop (𝓝 0) := by
  ...
  have hf := squeeze_zero' (hb.mono fun n hn => mul_nonneg (by positivity : 0 ≤ (n : ℝ) + 1) hn.1)
    ...
    (by simpa only [Function.comp_apply, Nat.cast_add, Nat.cast_one, mul_zero] using
      (tendsto_sqrt_div_nat.comp (tendsto_add_atTop_nat 1)).const_mul (128 * ((k : ℝ) + 1) ^ (k + 2)))
  have ho := squeeze_zero'
    (Eventually.of_forall fun n : ℕ => mul_nonneg (by positivity : 0 ≤ (n : ℝ) + 1) (omittedTail_nonneg k n (k + 1)))
    ((eventually_ge_atTop 1).mono fun n hn => omittedTail_scaled_le k n hn)
    (by simpa only [Function.comp_apply, Nat.cast_add, Nat.cast_one, Real.sqrt_div_self', mul_one_div, mul_zero] using
      (tendsto_sqrt_div_nat.comp (tendsto_add_atTop_nat 1)).const_mul (128 * ((k + 1 : ℕ) : ℝ) ^ (k + 1)))
  simpa only [← mul_add, ← tailErr_eq, add_zero] using ho.add hf

theorem tendsto_tailErr (k : ℕ) : Tendsto (tailErr k) atTop (𝓝 0) := by
  simpa only [mul_div_cancel_left₀ _ (Nat.cast_add_one_ne_zero (R := ℝ) _)] using (tendsto_tailErr_mul k).div_atTop
...
theorem affine_exists_residue {d Q A : ℕ} (hd : 0 < d) (hcop : Nat.Coprime Q d) : ∃ v < d, d ∣ Q * v + A := by
  let : NeZero d := ⟨hd.ne'⟩
...
theorem affine_iff_modEq {d Q A v : ℕ} (hcop : Nat.Coprime Q d) (hv : d ∣ Q * v + A) (j : ℕ) : d ∣ Q * j + A ↔ j ≡ v [MOD d] := by
  constructor
  · intro hj
    exact Nat.ModEq.cancel_left_of_coprime hcop.symm.gcd_eq_one (Nat.ModEq.add_right_cancel' A (hj.modEq_zero_nat.trans hv.modEq_zero_nat.symm))
  · intro hj
...
theorem affine_reduced_residue {d Q A : ℕ} (hd : 0 < d) (hcompat : Nat.gcd d Q ∣ A) :
  ...
  have hcop : Nat.Coprime (Q / Nat.gcd d Q) (d / Nat.gcd d Q) := (Nat.coprime_div_gcd_div_gcd hg).symm
  obtain ⟨v, hvlt, hv⟩ := affine_exists_residue (A := A / Nat.gcd d Q) (Nat.div_pos (Nat.le_of_dvd hd hgd) hg) hcop
  refine ⟨v, hvlt, fun j => ?_⟩
  rw [← affine_iff_modEq hcop hv j, Nat.div_dvd_iff_dvd_mul hgd hg, Nat.mul_add, ← Nat.mul_assoc, Nat.mul_div_cancel' (Nat.gcd_dvd_right d Q),
    Nat.mul_div_cancel' hcompat]

/-- A solution necessarily satisfies the elementary gcd compatibility condition. -/
theorem affine_gcd_dvd {d Q A j : ℕ} (hj : d ∣ Q * j + A) : Nat.gcd d Q ∣ A := (Nat.dvd_add_iff_right (dvd_mul_of_dvd_left (Nat.gcd_dvd_right d Q) j)).mpr
...
def meanTerm (k Q A d : ℕ) : ℝ := if Nat.gcd d Q ∣ A then (Nat.gcd d Q : ℝ) / (d : ℝ) ^ (k + 1) else 0

def progMean (k Q A : ℕ) : ℝ := ∑' d : ℕ, meanTerm k Q A d

/-- The phase is the finite sum of its reciprocal divisor powers. -/
theorem hasSum_divTerm (k : ℕ) {n : ℕ} (hn : 0 < n) : HasSum (fun d => divTerm k d n) (phase k n) := by
  have hsum : phase k n = ∑ d ∈ n.divisors, divTerm k d n := by
    ...
    rw [divTerm, if_pos (Nat.dvd_of_mem_divisors hd), Nat.cast_div_charZero (Nat.dvd_of_mem_divisors hd), div_pow, div_right_comm,
...
theorem divTerm_sum_eq_count (k Q A d N : ℕ) : (∑ n ∈ Finset.range N, divTerm k d (Q * n + A)) / (N : ℝ) =
...
private theorem tendsto_natDiv_ratio {d : ℕ} (hd : 0 < d) : Tendsto (fun N : ℕ => ((N / d : ℕ) : ℝ) / (N : ℝ)) atTop (𝓝 (1 / (d : ℝ))) := by
  simpa only [Function.comp_def, one_div_mul_eq_div, Nat.floor_div_eq_div] using (tendsto_nat_floor_mul_div_atTop (a := (1 : ℝ) / (d : ℝ))
...
private theorem tendsto_count_modEq_div {r : ℕ} (hr : 0 < r) (v : ℕ) : Tendsto (fun N : ℕ => (Nat.count (· ≡ v [MOD r]) N : ℝ) / (N : ℝ))
      atTop (𝓝 (1 / (r : ℝ))) := by
  have hε : Tendsto (fun N : ℕ => (if v % r < N % r then (1 : ℝ) else 0) / (N : ℝ)) atTop (𝓝 0) := by
    refine squeeze_zero (fun N => by positivity) (fun N => ?_) (tendsto_one_div_atTop_nhds_zero_nat (𝕜 := ℝ))
    gcongr
    split_ifs <;> norm_num
  have key (N : ℕ) : (Nat.count (· ≡ v [MOD r]) N : ℝ) / (N : ℝ) = ((N / r : ℕ) : ℝ) / (N : ℝ) + (if v % r < N % r then (1 : ℝ) else 0) / (N : ℝ) := by
...
theorem divTerm_cesaro {k : ℕ} (hk : 0 < k) (Q A d : ℕ) : Tendsto (fun N : ℕ =>
  ...
  · have hg : 0 < Nat.gcd d Q := Nat.gcd_pos_of_pos_left Q hd
    ...
    rw [show (Nat.gcd d Q : ℝ) / (d : ℝ) ^ (k + 1) = 1 / ((d / Nat.gcd d Q : ℕ) : ℝ) / (d : ℝ) ^ k by
      rw [Nat.cast_div hgd (Nat.cast_ne_zero.mpr hg.ne'), one_div_div, div_div, pow_succ, mul_comm ((d : ℝ) ^ k)]]
...
theorem affine_count_le_quotient {Q A d : ℕ} (hQ : 0 < Q) (hA : 0 < A) (N : ℕ) : ((Finset.range N).filter (fun n => d ∣ Q * n + A)).card ≤ (Q * N + A) / d := by
  rw [← Nat.Ioc_filter_dvd_card_eq_div]
  refine Finset.card_le_card_of_injOn (fun n => Q * n + A) (fun n hn => ?_) (fun n _ m _ h => Nat.eq_of_mul_eq_mul_left hQ (Nat.add_right_cancel h))
...
theorem divTerm_cesaro_bound {k Q A : ℕ} (hk : 0 < k) (hQ : 0 < Q) (hA : 0 < A) (d N : ℕ) : ‖(∑ n ∈ Finset.range N, divTerm k d (Q * n + A)) / (N : ℝ)‖ ≤
      ((Q + A : ℕ) : ℝ) / (d : ℝ) ^ (k + 1) := by
  ...
  have hmul : (((Finset.range N).filter (fun n => d ∣ Q * n + A)).card : ℝ) * d ≤ ((Q + A : ℕ) : ℝ) * N := by
    have hcard := affine_count_le_quotient hQ hA N (d := d)
    exact_mod_cast (Nat.mul_le_mul_right d hcard).trans ((Nat.div_mul_le_self _ _).trans (by nlinarith))
  rw [div_div, Real.norm_eq_abs, abs_of_nonneg (by positivity), pow_succ, div_le_div_iff₀ (by positivity) (by positivity)]
  nlinarith [mul_le_mul_of_nonneg_left hmul (pow_nonneg hdR.le k)]

theorem meanTerm_bounds {k Q : ℕ} (hQ : 0 < Q) (A d : ℕ) : 0 ≤ meanTerm k Q A d ∧ meanTerm k Q A d ≤ (Q : ℝ) / (d : ℝ) ^ (k + 1) := by
  unfold meanTerm
...
theorem summable_meanTerm {k Q : ℕ} (hk : 0 < k) (hQ : 0 < Q) (A : ℕ) : Summable (meanTerm k Q A) :=
  Summable.of_nonneg_of_le (fun d => (meanTerm_bounds hQ A d).1) (fun d => (meanTerm_bounds hQ A d).2)
    (by simpa only [mul_one_div] using (Real.summable_one_div_nat_pow.mpr (by omega : 1 < k + 1)).mul_left (Q : ℝ))

theorem one_le_progMean {k Q : ℕ} (hk : 0 < k) (hQ : 0 < Q) (A : ℕ) : 1 ≤ progMean k Q A := by
  have hh := (summable_meanTerm hk hQ A).le_tsum 1 (fun d _ => (meanTerm_bounds hQ A d).1)
  simpa [progMean, meanTerm] using hh

/-- The actual phase averages along any positive arithmetic progression. -/
theorem tendsto_progMean {k Q A : ℕ} (hk : 0 < k) (hQ : 0 < Q) (hA : 0 < A) : Tendsto (fun N : ℕ =>
      (∑ n ∈ Finset.range N, phase k (Q * n + A)) / (N : ℝ)) atTop
      (𝓝 (progMean k Q A)) := by
  have hs : Summable (fun d : ℕ => ((Q + A : ℕ) : ℝ) / (d : ℝ) ^ (k + 1)) := by
    simpa only [mul_one_div] using (Real.summable_one_div_nat_pow.mpr (by omega : 1 < k + 1)).mul_left ((Q + A : ℕ) : ℝ)
...
theorem fresh_prime_residues {ι : Type*} [Fintype ι] (r : ι → ℕ) (i₀ : ι) (Q A : ℕ) (hQ : 0 < Q) (hrpos : ∀ i, 0 < r i) :
    ∃ L : ℕ, L.Prime ∧ Q.Coprime L ∧ ∃ v₀ v₁ : ℕ, (∀ i, ¬ L ∣ Q * v₀ + A + r i) ∧ (∀ i, L ∣ Q * v₁ + A + r i ↔ r i = r i₀) := by
  classical
  ...
  · exact (Nat.ModEq.add_left_cancel' _ (hi.modEq_zero_nat.trans hv₁.modEq_zero_nat.symm)).eq_of_lt_of_lt (hrL i) (hrL i₀)
...
theorem tendsto_cesaro_sub {f : ℕ → ℝ} {a : ℝ} (hf : Tendsto f atTop (𝓝 a)) {L : ℕ} (hL : 0 < L) (v : ℕ) :
    Tendsto (fun N : ℕ => (∑ n ∈ Finset.range N, f (L * n + v)) / (N : ℝ)) atTop (𝓝 a) := by
  simpa only [div_eq_mul_inv, mul_comm, Function.comp_def, id_eq] using (hf.comp ((tendsto_add_atTop_nat v).comp (tendsto_id.const_mul_atTop' hL))).cesaro

/-- An isolated shift contributes exactly its own weighted change in mean. -/
theorem mean_difference {ι : Type*} [Fintype ι] (r : ι → ℕ) (c μ : ι → ℝ) (i₀ : ι) (β κ : ℝ) (hunique : ∀ i, r i = r i₀ → i = i₀) :
    (∑ i, c i * (μ i * (β + if r i = r i₀ then κ else 0))) - (∑ i, c i * (μ i * β)) = c i₀ * μ i₀ * κ := by
  classical
...
theorem gcd_eq_prime_or_one {ell : ℕ} (hp : ell.Prime) (d : ℕ) : Nat.gcd d ell = if ell ∣ d then ell else 1 := by
  by_cases h : ell ∣ d
  · simp [h, Nat.gcd_eq_right h]
  · simpa [h] using (hp.coprime_iff_not_dvd.mpr h).symm.gcd_eq_one

theorem meanTerm_mul (k : ℕ) {Q ell : ℕ} (hc : ell.Coprime Q) (A d : ℕ) : meanTerm k Q A (ell * d) = meanTerm k Q A d / (ell : ℝ) ^ (k + 1) := by
...
theorem progMean_multiples (k : ℕ) {Q ell : ℕ} (hell : 0 < ell) (hc : ell.Coprime Q) (A : ℕ) :
  ...
  simpa [meanTerm_mul k hc, progMean, tsum_div_const] using ((mul_right_injective₀ hell.ne').tsum_eq hs).symm

theorem meanTerm_refine (k : ℕ) {Q ell : ℕ} (hp : ell.Prime) (hc : ell.Coprime Q) (B d : ℕ) : meanTerm k (Q * ell) B d = meanTerm k Q B d +
  ...
  · simp only [hd, ↓reduceIte]
    have hprod : Nat.gcd d Q * ell ∣ B ↔ Nat.gcd d Q ∣ B ∧ ell ∣ B := by
      exact ⟨fun h => ⟨dvd_of_mul_right_dvd h, dvd_of_mul_left_dvd h⟩, fun h => (hc.symm.gcd_left d).mul_dvd_of_dvd_of_dvd h.1 h.2⟩
    by_cases hg : Nat.gcd d Q ∣ B <;> by_cases hB : ell ∣ B <;> simp only [hg, hB, hprod, and_self, and_false, false_and, ↓reduceIte]
    all_goals push_cast; ring
  · simp only [hd, ↓reduceIte, Nat.mul_one, mul_zero, add_zero]

theorem progMean_congr (k : ℕ) {Q A B : ℕ} (hAB : Nat.ModEq Q B A) : progMean k Q B = progMean k Q A :=
...
theorem progMean_refine {k Q A B ell : ℕ} (hk : 0 < k) (hQ : 0 < Q) (hp : ell.Prime) (hc : ell.Coprime Q) (hAB : Nat.ModEq Q B A) :
    progMean k (Q * ell) B = progMean k Q A * (1 - 1 / (ell : ℝ) ^ (k + 1) + if ell ∣ B then 1 / (ell : ℝ) ^ k else 0) := by
  have hs := summable_meanTerm hk hQ B
  have hm : Summable (fun d => if ell ∣ d then meanTerm k Q B d else 0) := hs.summable_of_eq_zero_or_self (fun d => by by_cases h : ell ∣ d <;> simp [h])
...
theorem tendsto_refined_mean {k Q : ℕ} (hk : 0 < k) (hQ : 0 < Q) (A L v : ℕ) (hp : L.Prime) (hcop : L.Coprime Q) (hA : 0 < A) : Tendsto (fun N : ℕ =>
      (∑ n ∈ Finset.range N, phase k (Q * (L * n + v) + A)) / (N : ℝ)) atTop
      (𝓝 (progMean k Q A * (1 - 1 / (L : ℝ) ^ (k + 1) + if L ∣ Q * v + A then 1 / (L : ℝ) ^ k else 0))) := by
...
theorem isolated_shift_not_tendsto_zero {k : ℕ} (hk : 0 < k) {ι : Type*} [Fintype ι] (r : ι → ℕ) (c : ι → ℝ) (i₀ : ι) (Q A : ℕ)
  ...
  have hmean (v : ℕ) (i : ι) : Tendsto (fun N : ℕ =>
      ...
      (𝓝 (progMean k Q (A + r i) * (1 - 1 / (L : ℝ) ^ (k + 1) + if L ∣ Q * v + A + r i then 1 / (L : ℝ) ^ k else 0))) := by
    simpa only [Nat.add_assoc] using tendsto_refined_mean hk hQ (A + r i) L v hp hcop.symm (by have := hrpos i; omega)
  have heq (v : ℕ) := tendsto_nhds_unique (tendsto_weightedCesaro _ c _ (hmean v)) (tendsto_cesaro_sub hz hp.pos v)
  have h₀ := heq v₀
  have h₁ := heq v₁
  simp only [hmiss, hhit, ↓reduceIte, add_zero] at h₀ h₁
  have hdiff := mean_difference r c (fun i => progMean k Q (A + r i)) i₀ (1 - 1 / (L : ℝ) ^ (k + 1)) (1 / (L : ℝ) ^ k) hunique
  rw [h₀, h₁, sub_self] at hdiff
  exact (mul_ne_zero (mul_ne_zero hc (lt_of_lt_of_le one_pos (one_le_progMean hk hQ (A + r i₀))).ne') (by positivity)) hdiff.symm
...
def diffWeight (order : ℕ) (i : Fin (order + 1)) : ℤ := (-1) ^ (order - (i : ℕ)) * Nat.choose order (i : ℕ)

/-- All polynomial moments below the difference order vanish. -/
theorem diffWeight_moment {order ell : ℕ} (hell : ell < order) (z d : ℝ) : (∑ i : Fin (order + 1), (diffWeight order i : ℝ) * (z + d * (i : ℕ)) ^ ell) = 0 := by
  have hh := congrFun (Polynomial.fwdDiff_iter_eq_zero_of_degree_lt (P := (Polynomial.C d * Polynomial.X + Polynomial.C z) ^ ell) (n := order)
...
def cubeWeight (order : ℕ) {n : ℕ} (e : Fin n → Fin (order + 1)) : ℤ := ∏ j : Fin n, diffWeight order (e j)

theorem cubeWeight_cons (order : ℕ) {n : ℕ} (a : Fin (order + 1)) (e : Fin n → Fin (order + 1)) :
    cubeWeight order (Fin.cons a e) = diffWeight order a * cubeWeight order e := by simp [cubeWeight, Fin.prod_univ_succ]

private theorem sum_cube_succ (order : ℕ) {n : ℕ} (G : (Fin (n + 1) → Fin (order + 1)) → ℝ) : (∑ e : Fin (n + 1) → Fin (order + 1), G e) =
      ∑ a : Fin (order + 1), ∑ e : Fin n → Fin (order + 1), G (Fin.cons a e) := by
  simpa only [Fintype.sum_prod_type, Fin.consEquiv, Equiv.coe_fn_mk] using ((Fin.consEquiv (fun _ : Fin (n + 1) => Fin (order + 1))).sum_comp G).symm

/-- A coordinate absent from the test argument kills every core of degree
below the difference order. -/
theorem cube_core {order ell : ℕ} (hell : ell < order) {n : ℕ} (d c : Fin n → ℝ) (i : Fin n) (hc : c i = 0) (g : ℝ → ℝ) (p q : ℝ) :
    (∑ e : Fin n → Fin (order + 1), (cubeWeight order e : ℝ) * ((p + ∑ a, d a * (e a : ℕ)) ^ ell * g (q + ∑ a, c a * (e a : ℕ)))) = 0 := by
  induction n generalizing p q with
  | zero => exact i.elim0
  | succ n ih =>
    rw [sum_cube_succ]
    simp only [cubeWeight_cons, Int.cast_mul, Fin.sum_univ_succ (n := n), Fin.cons_zero, Fin.cons_succ, ← add_assoc]
    rcases Fin.eq_zero_or_eq_succ i with rfl | ⟨i, rfl⟩
    · rw [Finset.sum_comm]
      refine Finset.sum_eq_zero fun e _ => ?_
      have hm := congrArg (fun x : ℝ => x * ((cubeWeight order e : ℝ) * g (q + ∑ b : Fin n, c b.succ * (e b : ℕ))))
...
def gridWeightedIndex (k : ℕ) (e : GridVertex k) : ℕ := ∑ j : Fin k, (j.val + 1) * (e j).val * (k + 2) ^ j.val
...
theorem gridIndex_le (k : ℕ) (e : GridVertex k) : gridIndex k e ≤ gridBound k := Nat.le_sub_one_of_lt (finFunctionFinEquiv e).isLt

/-- Fixed-length radix encodings distinguish all vertices. -/
theorem gridIndex_injective (k : ℕ) : Function.Injective (gridIndex k) := fun _ _ h => finFunctionFinEquiv.injective (Fin.ext h)
...
theorem gridWeightedIndex_le (k : ℕ) (e : GridVertex k) : gridWeightedIndex k e ≤ k * gridIndex k e := by
  unfold gridWeightedIndex gridIndex
  rw [Finset.mul_sum]
  refine Finset.sum_le_sum fun j _ => ?_
  simpa only [mul_assoc] using Nat.mul_le_mul_right ((e j).val * (k + 2) ^ j.val) j.isLt

theorem gridMult_pos (k : ℕ) (e : GridVertex k) : 0 < gridMult k e := Nat.add_pos_left (Nat.add_pos_left Nat.one_pos _) _

/-- Each multiplier is one modulo the factorial spacing. -/
theorem gridMult_coprime_spacing (k : ℕ) (e : GridVertex k) : Nat.Coprime (gridMult k e) (gridSpacing k) := by
  simp only [gridMult, gridBase, Nat.coprime_add_mul_left_left, Nat.mul_right_comm k (gridSpacing k) (gridBound k),
    Nat.coprime_add_mul_right_left, Nat.coprime_one_left_eq_true]

private theorem multipliers_coprime_of_index_lt {k : ℕ} {e f : GridVertex k} (hef : gridIndex k e < gridIndex k f) :
  ...
  have hlF : l ≤ gridBound k := (Nat.le_of_dvd (Nat.sub_pos_of_lt hef) hdiff).trans ((Nat.sub_le _ _).trans (gridIndex_le k f))
...
theorem gridMult_pairwise_coprime (k : ℕ) : Pairwise (fun e f : GridVertex k =>
      Nat.Coprime (gridMult k e) (gridMult k f)) := by
  intro e f hef
...
theorem gridShift_pos (k : ℕ) (e : GridVertex k) (j : ℕ) : 0 < gridShift k e j := Nat.sub_pos_of_lt ((gridOffset_lt_base k e).trans_le
    ((Nat.le_add_right _ _).trans (Nat.le_mul_of_pos_left _ j.succ_pos)))

/-- Natural subtraction in the shift expression is exact. -/
theorem gridShift_add_offset (k : ℕ) (e : GridVertex k) (j : ℕ) : gridShift k e j + gridOffset k e = (j + 1) * gridMult k e := by
...
theorem gridShift_eq_succ_base_iff {k : ℕ} (e : GridVertex k) {j : ℕ} (hj : j ≤ k) : gridShift k e j = (k + 1) * gridBase k ↔ e = gridZero k ∧ j = k := by
  ...
  · intro he
    ...
    have hjk : j = k := by
      ...
      have hmul := Nat.mul_le_mul_right (1 + k * gridSpacing k * gridBound k + gridSpacing k * gridIndex k e) hlt
      nlinarith
    rw [hjk] at hs
    refine ⟨gridIndex_injective k ?_, hjk⟩
    rw [gridIndex_zero]
    exact (Nat.mul_eq_zero.mp (by nlinarith : gridSpacing k * gridIndex k e = 0)).resolve_left (Nat.factorial_ne_zero _)
  · rintro ⟨rfl, rfl⟩
    simp [gridShift, gridMult, gridOffset, gridIndex, gridWeightedIndex, gridZero]

theorem gridModulus_pos (k : ℕ) : 0 < gridModulus k := Finset.prod_pos (fun e _ => pow_pos (gridMult_pos k e) 2)

/-- All prescribed offset congruences have a simultaneous squared-modulus solution. -/
theorem grid_exists_crt (k : ℕ) : ∃ N0 : ℕ, ∀ e : GridVertex k, N0 ≡ gridOffset k e [MOD (gridMult k e) ^ 2] := by
  classical
  let N0 := Nat.chineseRemainderOfFinset (gridOffset k) (fun e => (gridMult k e) ^ 2) Finset.univ (fun e _ => (pow_pos (gridMult_pos k e) 2).ne')
...
theorem gridShift_real_eq_cube (k : ℕ) (e : GridVertex k) (j : ℕ) : (gridShift k e j : ℝ) = ((j : ℝ) + 1) * gridBase k + ∑ i : Fin k, ((j : ℝ) - i.val) *
...
theorem grid_core_real_cancel {k j ell : ℕ} (hj : j < k) (hell : ell ≤ k) (g : ℝ → ℝ) : (∑ e : GridVertex k, (gridWeight k e : ℝ) *
      ((gridMult k e : ℝ) ^ ell * g (gridShift k e j))) = 0 := by
  simp_rw [gridMult_real_eq_cube, gridShift_real_eq_cube]
  exact cube_core (Nat.lt_succ_of_le hell) (fun i : Fin k => (gridSpacing k : ℝ) * ((k : ℝ) + 2) ^ i.val)
...
theorem grid_core_cancel {k j ell : ℕ} (hj : j < k) (hell : ell ≤ k) (g : ℕ → ℝ) : (∑ e : GridVertex k, (gridWeight k e : ℝ) *
      ((gridMult k e : ℝ) ^ ell * g (gridShift k e j))) = 0 := by simpa only [Nat.floor_natCast] using grid_core_real_cancel hj hell (fun z => g ⌊z⌋₊)

def gridCore (k N j i : ℕ) : ℝ := ∑ e : GridVertex k, (gridWeight k e : ℝ) * ((gridMult k e : ℝ) ^ (i + 1) * ((σ k (N + gridShift k e j) : ℝ) /
        ((N + gridShift k e j : ℕ) : ℝ) ^ (i + 1)))

def finiteMain (k N : ℕ) : ℝ := ∑ i ∈ Finset.range (k + 1), ∑ j ∈ Finset.range (k + 1), (Nat.stirlingSecond i j : ℝ) * gridCore k N j i

def gridCoeff (k : ℕ) (e : GridVertex k) (j : ℕ) : ℝ := (gridWeight k e : ℝ) * (gridMult k e : ℝ) ^ (k + 1) * (Nat.stirlingSecond k j : ℝ)
...
def survivor (k N : ℕ) : ℝ := ∑ e : GridVertex k, ∑ j ∈ Finset.range (k + 1), gridCoeff k e j * phase k (N + gridShift k e j)
...
theorem finiteMain_eq_final (k N : ℕ) : finiteMain k N = ∑ j ∈ Finset.range (k + 1), (Nat.stirlingSecond k j : ℝ) * gridCore k N j k := by
  unfold finiteMain
  ...
  · intro i hi hne
    ...
    by_cases hlt : j < k
    · rw [show gridCore k N j i = 0 from grid_core_cancel hlt (by omega) (fun s => (σ k (N + s) : ℝ) / ((N + s : ℕ) : ℝ) ^ (i + 1)), mul_zero]
...
theorem finiteMain_eq_surviving (k N : ℕ) : finiteMain k N = survivingMain k N := by
  have hdiv (x : ℕ) : (σ k x : ℝ) / (x : ℝ) ^ (k + 1) = phase k x / (x : ℝ) := by rw [phase, div_div, pow_succ]
...
theorem finiteMain_vertex (k N : ℕ) : finiteMain k N = ∑ e : GridVertex k, (gridWeight k e : ℝ) * (∑ j ∈ Finset.range (k + 1), ∑ i ∈ Finset.range (k + 1),
          (Nat.stirlingSecond i j : ℝ) * (gridMult k e : ℝ) ^ (i + 1) * (σ k (N + gridShift k e j) : ℝ) / ((N + gridShift k e j : ℕ) : ℝ) ^ (i + 1)) := by
...
theorem gridCoeff_zero_ne_zero (k : ℕ) : gridCoeff k (gridZero k) k ≠ 0 := by
  simp [gridCoeff, Nat.stirlingSecond_self, gridWeight, gridZero, cubeWeight, diffWeight, (gridMult_pos k (gridZero k)).ne']
...
theorem tendsto_phase_div_nat (k : ℕ) : Tendsto (fun n : ℕ => phase k n / (n : ℝ)) atTop (𝓝 0) := by
  ...
  simpa only [phase, mul_div_assoc] using div_le_div_of_nonneg_right (sigma_div_le_sqrt k n (by omega)) (Nat.cast_nonneg n : (0 : ℝ) ≤ n)

/-- Every fixed coefficient and shift preserves the vanishing ratio limit. -/
theorem grid_main_term_tendsto_zero (k : ℕ) (e : GridVertex k) (h : ℕ) : Tendsto (fun N : ℕ =>
      gridCoeff k e h * phase k (N + gridShift k e h) / ((N + gridShift k e h : ℕ) : ℝ)) atTop (𝓝 0) := by
...
theorem tendsto_survivingMain (k : ℕ) : Tendsto (survivingMain k) atTop (𝓝 0) := by
  ...
  simpa only [Finset.sum_const_zero] using tendsto_finsetSum (Finset.univ : Finset (GridVertex k)) (fun e _ => tendsto_finsetSum (Finset.range (k + 1))
...
theorem tendsto_survivor_rescaling (k : ℕ) : Tendsto (fun N : ℕ =>
      (N : ℝ) * survivingMain k N - survivor k N)
      atTop (𝓝 0) := by
  have hh := tendsto_finsetSum (Finset.univ : Finset (GridVertex k)) (fun e _ => tendsto_finsetSum (Finset.range (k + 1))
...
theorem tendsto_affine_atTop (Q A : ℕ) (hQ : 0 < Q) : Tendsto (fun t : ℕ => A + Q * t) atTop atTop :=
...
def GridCongruences (k N : ℕ) : Prop := ∀ e : GridVertex k, N ≡ gridOffset k e [MOD (gridMult k e) ^ 2]

/-- Exact conversion of a shifted quotient into the common shifted argument. -/
theorem tailIndex_factorization (k N : ℕ) (e : GridVertex k) (hN : gridOffset k e ≤ N) (hcong : N ≡ gridOffset k e [MOD (gridMult k e) ^ 2]) (j : ℕ) :
    gridMult k e * (tailIndex k N e + (j + 1)) = N + gridShift k e j := by
  have hindex : gridMult k e * tailIndex k N e + gridOffset k e = N := by
    unfold tailIndex
    rw [Nat.mul_comm, Nat.div_mul_cancel ((dvd_pow_self _ (by norm_num : (2 : ℕ) ≠ 0)).trans hcong.symm.dvd'), Nat.sub_add_cancel hN]
  have hshift := gridShift_add_offset k e j
  nlinarith

/-- Every retained actual shift is coprime to the grid multiplier. -/
theorem index_coprime {k : ℕ} (hk : 0 < k) (N : ℕ) (e : GridVertex k) (hcong : N ≡ gridOffset k e [MOD (gridMult k e) ^ 2]) {j : ℕ} (hj : j ≤ k) :
    Nat.Coprime (gridMult k e) (tailIndex k N e + (j + 1)) := by
  rw [Nat.coprime_add_iff_right (show gridMult k e ∣ tailIndex k N e from Nat.dvd_div_of_mul_dvd (by simpa only [pow_two] using hcong.symm.dvd'))]
  exact (gridMult_coprime_spacing k e).coprime_dvd_right (Nat.dvd_factorial j.succ_pos ((by omega : j + 1 ≤ k + 1).trans (gridBound_succ_le hk)))

/-- Exact numerator and denominator rescaling for every denominator exponent. -/
theorem tailIndex_term_rescale {k : ℕ} (hk : 0 < k) (N : ℕ) (e : GridVertex k) (hN : gridOffset k e ≤ N) (hcong : N ≡ gridOffset k e [MOD (gridMult k e) ^ 2])
    {j : ℕ} (hj : j ≤ k) (i : ℕ) :
    (σ k (gridMult k e) : ℝ) * ((σ k (tailIndex k N e + (j + 1)) : ℝ) / ((tailIndex k N e + (j + 1) : ℕ) : ℝ) ^ (i + 1)) =
      (gridMult k e : ℝ) ^ (i + 1) * (σ k (N + gridShift k e j) : ℝ) / ((N + gridShift k e j : ℕ) : ℝ) ^ (i + 1) := by
  have hp : (gridMult k e : ℝ) ≠ 0 := Nat.cast_ne_zero.mpr (gridMult_pos k e).ne'
  rw [← tailIndex_factorization k N e hN hcong j, ArithmeticFunction.isMultiplicative_sigma.map_mul_of_coprime (index_coprime hk N e hcong hj)]
  push_cast
  rw [mul_pow, mul_div_mul_left _ _ (pow_ne_zero (i + 1) hp), mul_div_assoc]

/-- The congruences persist along the entire CRT progression. -/
theorem gridCongruences_add_modulus_mul (k N0 t : ℕ) (hN0 : GridCongruences k N0) : GridCongruences k (N0 + gridModulus k * t) := by
  intro e
  simpa only [Nat.add_zero] using (hN0 e).add ((dvd_mul_of_dvd_left (show (gridMult k e) ^ 2 ∣ gridModulus k from
        Finset.dvd_prod_of_mem (fun f : GridVertex k => (gridMult k f) ^ 2) (Finset.mem_univ e)) t).modEq_zero_nat)

def weightedTail (k N : ℕ) : ℝ := ∑ e : GridVertex k, (gridWeight k e : ℝ) * (σ k (gridMult k e) : ℝ) * scaledTail k (tailIndex k N e + 1)

def weightedMain (k N : ℕ) : ℝ := ∑ e : GridVertex k, (gridWeight k e : ℝ) * (σ k (gridMult k e) : ℝ) * tailMain k (tailIndex k N e)

def weightedError (k N : ℕ) : ℝ := ∑ e : GridVertex k, (gridWeight k e : ℝ) * (σ k (gridMult k e) : ℝ) * tailErr k (tailIndex k N e)
...
theorem tendsto_tailIndex (k Q A : ℕ) (hQ : 0 < Q) (e : GridVertex k) : Tendsto (fun t : ℕ => tailIndex k (A + Q * t) e) atTop atTop :=
  (Nat.tendsto_div_const_atTop (gridMult_pos k e).ne').comp ((tendsto_sub_atTop_nat (gridOffset k e)).comp (tendsto_affine_atTop Q A hQ))
...
theorem eventually_weightedTail_integral_prog (k : ℕ) (hx : ¬ Irrational (alpha k)) (A : ℕ) :
  ...
  have hlarge : ∀ᶠ t : ℕ in atTop, ∀ e : GridVertex k, K ≤ tailIndex k (A + gridModulus k * t) e :=
    eventually_all.mpr (fun e => (tendsto_tailIndex k _ A (gridModulus_pos k) e).eventually (eventually_ge_atTop K))
  filter_upwards [hlarge] with t ht
  choose z hz using fun e : GridVertex k => hK _ ((ht e).trans (Nat.le_succ _))
  exact ⟨∑ e : GridVertex k, gridWeight k e * (σ k (gridMult k e) : ℤ) * z e, by simp only [weightedTail, hz, Int.cast_sum, Int.cast_mul, Int.cast_natCast]⟩

/-- Cancellation identifies the actual weighted main term with the actual survivor main term. -/
theorem weightedMain_eq_surviving {k : ℕ} (hk : 0 < k) (N : ℕ) (hN : gridBase k ≤ N) (hcong : GridCongruences k N) : weightedMain k N = survivingMain k N := by
  ...
  simpa only [mul_assoc, mul_left_comm, mul_div_assoc] using congrArg ((Nat.stirlingSecond i j : ℝ) * ·) (tailIndex_term_rescale hk N e
...
theorem tailIndex_common_le (k N : ℕ) (e : GridVertex k) (hN : gridOffset k e ≤ N) (hcong : N ≡ gridOffset k e [MOD (gridMult k e) ^ 2]) :
...
theorem tendsto_grid_vertex_error_mul (k A : ℕ) (hA : GridCongruences k A) (e : GridVertex k) : Tendsto (fun t : ℕ => ((A + gridModulus k * t : ℕ) : ℝ) *
  ...
  have hnonneg := (hindex.eventually (eventually_ge_atTop (k + 1))).mono fun t ht => tailErr_nonneg k _ ht
  refine squeeze_zero' (hnonneg.mono fun t ht => mul_nonneg (Nat.cast_nonneg _) ht) ?_ hlim
  filter_upwards [hnonneg, (tendsto_affine_atTop _ A (gridModulus_pos k)).eventually (eventually_ge_atTop (gridOffset k e))] with t ht hN
  simpa only [mul_assoc] using mul_le_mul_of_nonneg_right (tailIndex_common_le k _ e hN (gridCongruences_add_modulus_mul k A t hA e)) ht

/-- Fixed signed weights preserve the vanishing scaled-error limit. -/
theorem tendsto_weightedError_mul (k A : ℕ) (hA : GridCongruences k A) : Tendsto (fun t : ℕ => ((A + gridModulus k * t : ℕ) : ℝ) *
      weightedError k (A + gridModulus k * t)) atTop (𝓝 0) := by
  simpa only [weightedError, Finset.mul_sum, mul_left_comm, mul_assoc, mul_zero, Finset.sum_const_zero] using
    tendsto_finsetSum (Finset.univ : Finset (GridVertex k)) (fun e _ =>
      (tendsto_grid_vertex_error_mul k A hA e).const_mul ((gridWeight k e : ℝ) * (σ k (gridMult k e) : ℝ)))

theorem tendsto_weightedError (k A : ℕ) (hA : GridCongruences k A) : Tendsto (fun t : ℕ => weightedError k (A + gridModulus k * t)) atTop (𝓝 0) := by
  have hN := (tendsto_natCast_atTop_atTop (R := ℝ)).comp (tendsto_affine_atTop (gridModulus k) A (gridModulus_pos k))
...
theorem eventually_main_eq_surviving {k : ℕ} (hk : 0 < k) (A : ℕ) (hA : GridCongruences k A) : (fun t : ℕ => weightedMain k (A + gridModulus k * t)) =ᶠ[atTop]
      fun t : ℕ => survivingMain k (A + gridModulus k * t) := by
  filter_upwards [(tendsto_affine_atTop _ A (gridModulus_pos k)).eventually (eventually_ge_atTop (gridBase k))] with t ht
  exact weightedMain_eq_surviving hk _ ht (gridCongruences_add_modulus_mul k A t hA)

theorem tendsto_weightedTail {k : ℕ} (hk : 0 < k) (A : ℕ) (hA : GridCongruences k A) : Tendsto (fun t : ℕ => weightedTail k (A + gridModulus k * t))
      atTop (𝓝 0) := by
  have hmain := ((tendsto_survivingMain k).comp (tendsto_affine_atTop _ A (gridModulus_pos k))).congr' (eventually_main_eq_surviving hk A hA).symm
...
theorem tendsto_survivor_of_rational {k : ℕ} (hk : 0 < k) (hx : ¬ Irrational (alpha k)) (A : ℕ) (hA : GridCongruences k A) :
    Tendsto (fun t : ℕ => survivor k (A + gridModulus k * t)) atTop (𝓝 0) := by
  have hz := eventually_zero_of_int (tendsto_weightedTail hk A hA) (eventually_weightedTail_integral_prog k hx A)
  have hmain : Tendsto (fun t : ℕ => ((A + gridModulus k * t : ℕ) : ℝ) * survivingMain k (A + gridModulus k * t)) atTop (𝓝 0) := by
    rw [← neg_zero]
...
theorem survivor_not_tendsto_zero {k : ℕ} (hk : 0 < k) (Q A : ℕ) (hQ : 0 < Q) : ¬ Tendsto (fun n : ℕ => survivor k (Q * n + A)) atTop (𝓝 0) := by
  let i₀ : GridTerm k := ⟨gridZero k, Fin.last k⟩
  have hunique (i : GridTerm k) (hi : gridShift k i.1 i.2 = gridShift k i₀.1 i₀.2) : i = i₀ := by
    obtain ⟨he, hj⟩ := (gridShift_eq_succ_base_iff i.1 (Nat.lt_succ_iff.mp i.2.isLt)).mp (hi.trans ((gridShift_eq_succ_base_iff i₀.1 le_rfl).mpr ⟨rfl, rfl⟩))
    exact Prod.ext he (Fin.ext hj)
  simpa only [Fintype.sum_prod_type, survivor, ← Fin.sum_univ_eq_sum_range] using isolated_shift_not_tendsto_zero hk
...
theorem tendsto_scaledTail_zero : Tendsto (scaledTail 0) atTop (𝓝 0) := by
  ...
  have hmain : Tendsto (fun n => tailMain 0 n) atTop (𝓝 0) := by
    simpa [tailMain_eq_range, phase, Function.comp_def] using (tendsto_phase_div_nat 0).comp (tendsto_add_atTop_nat 1)
